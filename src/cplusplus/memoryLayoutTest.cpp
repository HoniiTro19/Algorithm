...
int main() {
  char *s1 =
      "abcde"; //"abcde"作为字符串常量存储在常量区 s1、s2、s5拥有相同的地址
  char *s2 = "abcde";
  char s3[] = "abcd";
  long int *s4[100];
  char *s5 = "abcde";
  ...
  printf("variables address in main function: s1=%p  s2=%p s3=%p s4=%p s5=%p "
         "a=%p b=%p \n",
         s1, s2, s3, s4, s5, &a, &b);
...
void print(char *str, int p) {
  ...
  char *s2 =
      "abcde"; // abcde在常量区，s2在栈上
               // s2-s1=6可能等于0，编译器优化了相同的常量，只在内存保存一份
  // 而&s1>&s2
  char s3[] =
      "abcdeee"; // abcdeee在常量区，s3在栈上，数组保存的内容为abcdeee的一份拷贝
  long int *s4[100];
  char *s5 = "abcde";
  int a = 5;
  int b = 6;
  ...
  printf("s1=%p s2=%p s3=%p s4=%p s5=%p a=%p b=%p c=%p d=%p str=%p q=%p p=%p "
         "m=%p r=%p w=%p \n",
